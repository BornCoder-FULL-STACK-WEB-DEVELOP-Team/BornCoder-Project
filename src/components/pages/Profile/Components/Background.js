...
import {
  ...
  avatar,
  updatebutton,
} from "../Styles/Background.module.scss";
...
class Background extends React.Component {
  ...
  componentDidMount() {
    //网络请求都是放在此生命周期函数中
    fetch("http://localhost:8090/user/" + this.props.userId)
      ...
      <Fragment>
        <div className={background_container}>
          ...
          <IconButton
            ...
            className={classes.update_button + " " + cloud_upload_background}
            style={{ display: this.state.display }}
          >
            <CloudUploadIcon />
          </IconButton>
          <Button
            onClick={this.uploadFile_background}
            variant="contained"
            className={upload_button_background}
            style={{ display: this.state.display }}
          >
            Upload
          </Button>
        </div>
        <div className={info_update} style={{ display: this.state.display }}>
